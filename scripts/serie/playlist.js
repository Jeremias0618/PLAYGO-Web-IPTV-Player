    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function checkIfInPlaylist(playlistName) {
            ...
            for (const item of playlists[playlistName]) {
                if (item.id == serieId && item.type == 'serie') {
                    return true;
                }
            }
            return false;
        ...
        function handlePlaylistSelect(playlistName) {
            ...
            fetch('libs/endpoints/UserData.php', {
                method: 'POST',
                headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                body: `action=${action}&playlist_name=${encodeURIComponent(playlistName)}&id=${serieId}&nombre=${encodeURIComponent(serieName)}&img=${encodeURIComponent(serieImg)}&backdrop=${encodeURIComponent(serieBackdrop)}&ano=${encodeURIComponent(serieYear)}&rate=${encodeURIComponent(serieRating)}&tipo=serie`
            })
            .then(res => res.json())
            .then(data => {
                if (data.success) {
